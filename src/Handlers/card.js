...
module.exports = CardHandler = async (client, M) => {
  try {
    ...
    if (!Array.isArray(cardgame)) return; // Check if cardgame is an array
    ...
    for (let i = 0; i < cardgame.length; i++) {
      ...
      const job = cron.schedule('*/2 * * * *', async () => { // Schedule cron job
        try {
      ...
      const deleteJob = cron.schedule('*/1 * * * *', async () => { // Schedule cron job for deleting card
        await client.cards.delete(`${jid}.card`);
        await client.cards.delete(`${jid}.card_price`);
        console.log(`Card deleted after 5 minutes`);
        deleteJob.destroy(); // Destroy the cron job after execution
      });
    }
