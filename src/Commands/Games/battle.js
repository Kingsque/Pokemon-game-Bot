module.exports = {
  ...
  async execute(client, arg, M) {
    const battleInProgress = await client.DB.get(`${M.from}.battleInProgress`) || true;
    let player1, player2;
    ...
    if (arg === "challenge") {
      ...
      // Calculate health
      const player1Health = await client.DB.get(`${player1}.health`) || 100;
      const player2Health = await client.DB.get(`${player2}.health`) || 100;

      const newPlayer1Health = player1Health - player2Damage;
      const newPlayer2Health = player2Health - player1Damage;

      // Update health
      await client.DB.set(`${player1}.health`, newPlayer1Health);
      await client.DB.set(`${player2}.health`, newPlayer2Health);
      ...
      if (newPlayer1Health <= 0) {
        ...
        resultMessage = `Battle Update\n@${player1.split("@")[0]} health = ${newPlayer1Health}\n@${player2.split("@")[0]} health = ${newPlayer2Health}\nKeep battling with :battle attack`;
      }
